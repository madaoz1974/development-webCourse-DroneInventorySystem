...
public class CenterInfoController extends AbstractController {
	...
	public String newRegistrationExecution(
		Model model,
		@Valid CenterNewRegistrationForm form,
	 ...
	 * @param form
	 */
	private void handleValidationErrorForRegistration(
		Model model,
		BindingResult bindingResult,
		CenterNewRegistrationForm form
	) {
		// エラーメッセージをリストに格納
		StringBuilder errorMsg = new StringBuilder();
		...
		// 稼働ステータス不正文字列のバリデーション
			...
			bindingResult.rejectValue(
				"operationalStatus",
				"requiredField.empty",
				ErrorMessageHelper.getMessage(ErrorMessage.REQUIRED_ERROR_MESSAGE)
			);
		}
		// 備考不正文字列のバリデーション
	...
	private static boolean isInvalidOperationalStatus(int input) {
		return (
			input > ModelAttributeContents.MAX_OPERATIONAL_STATUS ||
			input < ModelAttributeContents.MIN_OPERATIONAL_STATUS
		);
	}
}
